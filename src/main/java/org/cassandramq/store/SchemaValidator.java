...
public final class SchemaValidator {
    ...
    public static void validateRequiredTables(CqlSession session) {
        boolean hasMessages = session.getMetadata().getKeyspace(session.getKeyspace().orElseThrow())
                .flatMap(ks -> ks.getTable("queue_messages_by_shard"))
                .isPresent();
        boolean hasOwnership = session.getMetadata().getKeyspace(session.getKeyspace().orElseThrow())
